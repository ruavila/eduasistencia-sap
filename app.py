...
if menu == "📚 Cursos":
    ...
    if not df_c.empty:
        ...
        if f and st.button("Importar y Generar QR"):
    ...
    st.subheader("Reporte de Asistencia Profesional")
    df_c = pd.read_sql("SELECT grado, materia FROM cursos WHERE profe_id=?", conn, params=(st.session_state.user,))
    if not df_c.empty:
        ...
        if st.button("📄 Generar Planilla PDF", type="primary", use_container_width=True):
            ...
            canv.save(); st.download_button("📥 Descargar Planilla", pdf_io.getvalue(), f"Reporte_{gr}.pdf", use_container_width=True)
