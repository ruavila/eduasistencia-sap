...
if menu == "📚 Mis Cursos":
    ...
    if not df_c.empty:
        ...
        if file and st.button("Generar QRs (4x4)", use_container_width=True):
            ...
            for _, row in df.iterrows():
                # Limpieza de ID: Texto puro sin decimales
    ...
    if not df_c.empty:
        ...
        if not tema:
            ...
            # Llave única para que la cámara no se bloquee
            cam_key = f"scan_{ga}_{len(tema)}"
            ...
            if codigo:
                ...
                if res:
                    ...
                    else: st.info(f"{nom_est} ya está en la lista.")
                else: st.error(f"ID {id_q} no pertenece a este grado.")
        ...
        if st.button("🚀 Finalizar y Notificar Ausentes", type="primary", use_container_width=True):
            try:
                ...
                # 1. Obtener lista completa ordenada
                ...
                # 2. Obtener quienes asistieron HOY (solo IDs)
                ...
                # 3. Filtrar ausentes de forma segura
                aus = todos[~todos['documento'].astype(str).isin(pres_list)]
                
                if aus.empty:
                    st.balloons()
                    st.success("¡Asistencia completa! No hay ausentes hoy.")
                else:
                    st.warning(f"Se detectaron {len(aus)} ausencias. Notifique a continuación:")
                ...
                st.error(f"Error al procesar ausencias: {e}")
    ...
    if not df_c.empty:
        ...
        sel_r = st.selectbox("Ver reporte:", op_r)
        gr, mr = sel_r.split(" | ")
