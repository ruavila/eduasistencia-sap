...
# --- CONTROL DE ACCESO (REGISTRO RESTAURADO) ---
        ...
        if st.button("Registrar Cuenta"):
            if reg_nom and reg_usu and reg_pass:
                try:
                    ...
                    conn.commit(); st.success("✅ Registro exitoso. Ya puedes iniciar sesión.")
...
menu = st.sidebar.radio("Navegación", ["📚 Mis Cursos", "👤 Gestionar Estudiantes", "📷 Asistencia QR", "⚙️ Reinicio"])
conn = get_connection()
...
if menu == "📚 Mis Cursos":
                ...
                if st.button("Generar PDF con QRs"):
                    if 'estudiante_id' in df.columns and 'nombre' in df.columns:
                        ...
                        for _, row in df.iterrows():
                            ...
                            # Guardamos el número tal cual viene en el Excel
                            ...
                            canv.setFont("Helvetica-Bold", 8); canv.drawCentredString(x + 2*cm, y - 0.5*cm, f"{enom[:15]} | {g_sel if 'g_sel' in locals() else g_s}")
                            x += 5.2*cm
    ...
    if df_c.empty: st.warning("Cree un curso primero.")
        ...
        id_qr = qrcode_scanner(key="scanner_cam") # Cámara activa
        if id_qr:
        ...
        # --- SECCIÓN WHATSAPP CON CÓDIGO 57 AUTOMÁTICO ---
        st.divider()
        if st.button("Finalizar y Reportar Inasistencias", type="primary"):
            ...
            if ausentes.empty: st.success("🎉 Todos presentes.")
                ...
                for _, est in ausentes.iterrows():
                    ...
                    # Si el número tiene 10 dígitos (formato móvil Colombia), le ponemos el 57
                    if len(tel) == 10:
                        tel_final = "57" + tel
                    else:
                        tel_final = tel # Lo dejamos como esté si ya es largo
    ...
    if st.checkbox("Confirmar borrado") and st.button("LIMPIAR TODO"):
