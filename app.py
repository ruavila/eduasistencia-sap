...
if not st.session_state.logueado:
    ...
    with t1:
        u_l, p_l = st.text_input("Usuario", key="u_l"), st.text_input("Contraseña", type="password", key="p_l")
...
if menu == "📚 Cursos":
    ...
    st.subheader("Scanner y Notificación WhatsApp")
    df_c = pd.read_sql("SELECT grado, materia FROM cursos WHERE profe_id=?", conn, params=(st.session_state.user,))
    if not df_c.empty:
        sel_as = st.selectbox("Seleccione Clase:", [f"{r['grado']} | {r['materia']}" for _, r in df_c.iterrows()])
        ga, ma = sel_as.split(" | ")
        ...
        if tema:
            ...
            if cod:
                ...
                res = conn.execute("SELECT documento, nombre, whatsapp FROM estudiantes WHERE documento LIKE ? AND grado=? AND profe_id=?", (f"%{id_cl}%", ga, st.session_state.user)).fetchone()
                if res:
                    doc, nom, tel = res; hoy = datetime.now().strftime("%Y-%m-%d"); hora = datetime.now().strftime("%H:%M:%S")
                    if not conn.execute("SELECT id FROM asistencia WHERE estudiante_id=? AND fecha=? AND tema=?", (doc, hoy, tema)).fetchone():
                        conn.execute("INSERT INTO asistencia (estudiante_id, fecha, hora, grado, materia, tema, profe_id) VALUES (?,?,?,?,?,?,?)", (doc, hoy, hora, ga, ma, tema, st.session_state.user))
                        conn.commit(); st.success(f"✅ {nom} registrado")
                    
                    # BOTÓN DE WHATSAPP
                    if tel:
                        msg = f"Cordial saludo. El estudiante {nom} asistió a la clase de {ma} (Tema: {tema}) hoy {hoy} a las {hora}."
                        link = f"https://wa.me/57{tel}?text={urllib.parse.quote(msg)}"
                        st.markdown(f'<a href="{link}" target="_blank"><button style="background-color:#25d366; color:white; border:none; padding:10px; border-radius:5px; width:100%; cursor:pointer;">📲 Notificar Asistencia por WhatsApp</button></a>', unsafe_allow_html=True)

elif menu == "📊 Reportes":
    st.subheader("Generar Planilla Profesional")
    df_c = pd.read_sql("SELECT grado, materia FROM cursos WHERE profe_id=?", conn, params=(st.session_state.user,))
    if not df_c.empty:
        ...
        if st.button("📄 Descargar PDF", type="primary", use_container_width=True):
            ...
            canv.setFont("Helvetica", 9); x_info = margen + 2.5*cm
            canv.drawString(x_info, alto_pag - 1.7*cm, f"Asignatura: {mr}")
            canv.drawString(x_info, alto_pag - 2.1*cm, f"Grado: {gr}") # Grado junto a asignatura
            canv.drawString(x_info, alto_pag - 2.5*cm, f"Docente: {st.session_state.profe_nom}")
            ...
            canv.save(); st.download_button("📥 Descargar Planilla", pdf_io.getvalue(), f"Planilla_{gr}.pdf", use_container_width=True)

elif menu == "⚙️ Reinicio":
    if st.button("LIMPIAR DATOS DE ESTE DOCENTE"):
