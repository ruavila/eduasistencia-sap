...
# CABECERA INSTITUCIONAL (Pantalla)
...
if not st.session_state.logueado:
    ...
    with t2:
        nu, nn, np = st.text_input("Nuevo ID"), st.text_input("Nombre"), st.text_input("Clave", type="password")
...
if menu == "📚 Cursos":
    ...
    if not df_c.empty:
        ...
        if f and st.button("Procesar"):
            ...
            st.download_button("📥 Descargar Carnets", pdf.getvalue(), f"QRs_{gs}.pdf", use_container_width=True)
    ...
    if not df_c.empty:
        ...
        if st.button("📄 Generar Planilla", type="primary", use_container_width=True):
            ...
            h_row = 0.55*cm # Fila compacta como su modelo 
            
            # Cabecera Tabla
            canv.rect(x_curr, y_table, w_nom, h_row)
            canv.setFont("Helvetica-Bold", 8); canv.drawString(x_curr + 0.2*cm, y_table + 0.15*cm, "NOMBRE DEL ESTUDIANTE")
            
            x_h = x_curr + w_nom
            for f, t in clases:
                canv.rect(x_h, y_table, w_col, h_row)
                canv.saveState()
                canv.translate(x_h + (w_col/2) + 0.1*cm, y_table + 0.1*cm)
                canv.rotate(90)
                canv.setFont("Helvetica-Bold", 5)
                canv.drawString(0, 0, f"{f} | {t[:12]}") # Fecha y Tema 
                canv.restoreState()
                x_h += w_col
            ...
            canv.save()
            st.download_button("📥 Descargar Reporte", pdf_io.getvalue(), f"Reporte_{gr}.pdf", "application/pdf", use_container_width=True)
